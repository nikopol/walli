...
<?php } ?>
	...
	<script>
		...
		walli.prev).add("RIGHT",walli.next).add("UP",walli.up).add("DOWN",walli.down).add("PAGEUP",walli.pgup).add("PAGEDOWN",walli.pgdown).add("END",walli.last).add(["ESC","BACKSPACE"],walli.back).add("DOWN",function(){!n&&e.length&&walli.show(0)}).add(["?","H"],function(){css("#help","*active")}).add(["T"],fa).add(["PLUS","+"],function(){C(1)}).add(["MINUS","-"],function(){C(-1)});_("#bprev").onclick=walli.prev;_("#bnext").onclick=walli.next;_("#bplay").onclick=walli.toggleplay;_("#bthumb").onclick=walli.thumb;
		_("#bzip").onclick=walli.dlzip;G||(_("#bzip").className="hide");ga&&(N=_("#who"),K=_("#what"),K.onfocus=N.onfocus=function(){X(!1)},_("#comments").onclick=v,_("#bcom").onclick=walli.togglecom,_("#bsend").onclick=walli.sendcom);w?(_("#blogout").onclick=walli.logout,FormData?(_("#iupload").onchange=function(a){a=new FormData;for(var b=0,c=this.files,f,d=0;d<c.length;++d)f=c[d],b+=f.size,a.append("file"+d,f);if(confirm(loc.tpl("uploadfiles",{z:loc.size(b),nb:c.length}))){var e=new XMLHttpRequest;e.open("POST",
		"walli.php?!=img&&path="+r);e.onload=function(){if(200==e.status){var a=JSON.parse(e.responseText);osd.loc("uploaded",{nb:a.added});a.added&&l(r)}else osd.error("error "+e.status)};e.upload.onprogress=function(a){event.lengthComputable&&osd.set(a.loaded,a.total)};e.send(a)}},_("#bupload").onclick=function(){_("#iupload").click()}):css("#bupload",{diplay:"none"}),_("#bdel").onclick=walli.del,_("#bdiag").onclick=walli.switchdiag,_("#bflush").onclick=walli.flush,_("#bmkdir").onclick=walli.mkdir):a.admin&&
		(_("#blogin").onclick=walli.login);log.info("show on!");var d=_("#intro");if(d){var f=setTimeout(function(){css(d,"hide")},5E3);d.onclick=function(){clearTimeout(f);css(d,"hide")}}ea()||(B("thumb"),l("/"));hash.onchange(ea)},login:function(){document.location="?login"+document.location.hash},logout:function(){document.location="?logout"+document.location.hash},del:function(){if(w){var a=e.filter(function(a,b){return-1!=x.indexOf(b)});x.length?ajax({type:"POST",url:"?!=del",data:{files:a.join("*")},
		ok:function(a){osd.loc("deleted",{nb:a.deleted});a.deleted&&l(r)},error:osd.error}):osd.error(loc.noselection)}},switchdiag:function(){var a=_("#diag");a?document.body.removeChild(a):walli.diag()},diag:function(){w&&ajax({url:"?!=diag",data:{path:r},ok:function(a){var b=_("#diag"),c="<ul>",d;for(d in a.stats)c+='<li class="stat">'+("size"==d?loc.size(a.stats[d]):a.stats[d]+" "+d)+"</li>";for(d in a.checks)c+='<li class="'+(a.checks[d]?"ok":"bad")+'">'+d+(a.checks[d]?" enabled":" disabled")+"</li>";
		b||(b=document.createElement("div"),b.id="diag",b.onclick=function(){document.body.removeChild(b)},document.body.appendChild(b));b.innerHTML=c},error:osd.error})},flush:function(){w&&ajax({url:"?!=flush",ok:function(a){osd.loc("flushed",{nb:a.flushed})},error:osd.error})},mkdir:function(){var a;w&&(a=prompt(loc.mkdir))&&ajax({type:"POST",url:"?!=mkdir",data:{dir:a,path:r},ok:function(){l(r)},error:osd.error})},dlzip:function(){var a=x.length?e.filter(function(a,b){return-1!=x.indexOf(b)}):e;G&&a.length?
		(_("#bzip",loc.zip),ajax({type:"POST",url:"?!=zip",data:{files:a.join("*")},ok:function(a){document.location="?!=zip&zip="+a.zip;walli.zwap()},error:osd.error})):osd.loc("nozip")},zwap:function(a){void 0!=a&&(-1==x.indexOf(a)?x.push(a):x=x.filter(function(b){return b!=a}));x.length?(b("selected",loc.tpl("dlsel",{nb:x.length})),w&&(_("#bdel").className="")):(b("all",loc.dlall),w&&(_("#bdel").className="hide"))},thumb:function(){B("thumb")},show:function(a,b,c){e.length&&(k=0>a?e.length+a:a>=e.length?
		a%e.length:a,n||B("tof"),css("#mask","+active"),s(k,function(a,c){css("#mask","-active");n?h=1-h:b=0;W[h]={w:c.width,h:c.height};q.removeChild(m[h]);m[h].src=c.src;if(b)css(m[h],0>b?"left":"right"),g(h,b),q.appendChild(m[h]),g(h,0),css(m[h],"animated center"),css(m[1-h],"animated "+(0<b?"left":"right")),g(1-h,-b);else{css(m[h],"");var d=position("#diapo"+k);css(m[h],{width:d.width+"px",height:d.height+"px",left:d.left+"px",top:d.top+"px"});q.appendChild(m[h]);css(m[h],"animated");g(h,0)}L();V();fa();
		1<e.length&&s((k+1)%e.length,function(){})}),$(e[k]))},next:function(a){v(a);n?walli.show(k+1,1):y(1,0)},prev:function(a){v(a);n?walli.show(k-1,-1):y(-1,0)},down:function(a){v(a);n?walli.show(k+1,1):y(0,1)},up:function(a){v(a);n?walli.show(k-1,-1):y(0,-1)},pgdown:function(a){v(a);n?walli.show(k+5,1):y(0,5)},pgup:function(a){v(a);n?walli.show(k-5,-1):y(0,-5)},first:function(a){v(a);n?walli.show(0,-1):y(0,0,!0)},last:function(a){v(a);n?walli.show(-1,1):y(-1,-1,!0)},play:function(a){v(a);e.length&&(n||
		walli.show(k,0),B("tof"))},stop:function(a){v(a);B("thumb")},toggleplay:function(a){v(a);n?X(!Q):(F[r]||y(0,0,!0),F[r].o&&F[r].o.children[0].click(a))},togglecom:function(a){a&&a.stopPropagation();Z(!R)},sendcom:function(){1>N.value.length?(osd.loc("emptywho"),N.focus()):1>K.value.length?(osd.loc("emptywhat"),K.focus()):ja(N.value,K.value)},rmcom:function(a,b){ka(a,b)},back:function(){if(n)return B("thumb");var a=r.split("/");1<a.length&&l(a.slice(0,a.length-2).join("/"))},cd:function(a){B("thumb");
		l(a)}}}();
	</script>
	<script>
		ready(function(){
