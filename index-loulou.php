...
<head>
	...
	<style>
		...
		*{margin:0;padding:0}
		...
		#osd{display:hidden;position:absolute;top:10px;left:60px;right:20px;height:40px;z-index:0;color:#fff;font:bold 30px Arial;opacity:0;text-align:right;padding:0 10px;-webkit-transition:all 0.5s ease;-moz-transition:all 0.5s ease;transition:all 0.5s ease;font:bold 30px Satisfy;text-shadow:0 4px 3px rgba(0, 0, 0, 0.4), 0 8px 10px rgba(0, 0, 0, 0.1), 0 8px 9px rgba(0, 0, 0, 0.1)}
		...
		#path{display:inline}
		#diapos{position:absolute;top:40px;padding:5px 5px 0 5px;left:0;right:0;bottom:0;overflow:auto;padding:10px 5px 0 5px}
...
<?php } ?>
	...
	<script>
		...
		a.files.length+" files found");if(q.length){var s=q.replace(/[^\/]+\/$/,"/"),g=document.createElement("li");css(g,"diapo up loaded");g.setAttribute("title",loc.updir);g.onclick=function(){n(s)};e.appendChild(g);var j="",l="";q.split("/").forEach(function(a){a&&(j+=a+"/",l+="<button onclick=\"walli.cd('"+j+"')\">"+a+"</button>")});_("#path",l);f()}var m=function(a,h,s,v){var c=document.createElement("img");c.onload=function(){osd.inc();log.debug(a+" loaded");css(this.parentNode,"+loaded")};c.onclick=
		h;h=document.createElement("li");css(h,"diapo "+s);h.appendChild(c);h.setAttribute("title",b(a));void 0!=v&&(c.id="diapo"+v,(C||w)&&append(h,'<input type="checkbox" id="chk'+v+'" n="'+v+'" onchange="walli.zwap('+v+')"/><label for="chk'+v+'"></label>'));if((A[a]||[]).length)append(h,'<span class="minicom">'+(999<A[a].length?Math.floor(A[a].length/1E3)+"K+":A[a].length)+"</span>");e.appendChild(h);c.src="?!=mini&file="+encodeURIComponent(a)};k=a.files;N=a.dirs;A=a.coms;h&&h();osd.start(k.length+N.length);
		a.dirs.forEach(function(a){m(a,function(){n(a)},"dir")});a.files.forEach(function(a,h){m(a,function(){walli.show(h,0)},"",h)});a.files.length&&C&&c("all");O();d();w&&_("#diag")&&walli.diag()})}function e(a,h){if(P[a]){var b=p.clientWidth,c=p.clientHeight,s=P[a],d=s.h,e=s.w;e>b&&(e=b,d=Math.floor(e*(s.h/s.w)));d>c&&(d=c,e=Math.floor(d*(s.w/s.h)));css(j[a],{width:e+"px",height:d+"px",left:Math.floor((b-e)/2+b*h)+"px",top:Math.floor((c-d)/2)+"px"})}}function S(){J&&(D&&clearTimeout(D),D=setTimeout(walli.next,
		1E3*W))}function y(a){J!==a&&((J=a)?(S(),a=document.documentElement,a.requestFullscreen?a.requestFullscreen():a.mozRequestFullScreen?a.mozRequestFullScreen():a.webkitRequestFullScreen&&a.webkitRequestFullScreen(),css("#bplay","active"),osd.loc("play")):(D&&clearTimeout(D),D=!1,css("#bplay",""),osd.loc("stop")))}function t(a){G!==a&&(G=a,log.debug("switch to "+a+" mode"),l=!0,"tof"==G?(z&&(clearInterval(z),z=!1),css(E,"+active"),css("#thumb","-active")):"zik"!=G&&"movie"!=G&&(l=!1,y(!1),css(j[0],""),
		css(j[1],""),css(E,"-active"),css("#thumb","+active"),d()),O())}function T(a){var h=_("#diapo"+m).parentNode,b=_("#minicom"+m);b&&h.removeChild(b);0<a&&append(h,'<span id="minicom'+m+'" class="minicom">'+a+"</span>")}function Q(a,b){if(b||l&&K!==a)K=a,l&&(css(j[1-g],""),e(1-g,2)),a?(css("#bcom","+active"),css(E,"+com"+(b?"fix":"")),hash.set("com",1)):(css("#bcom","-active"),css(E,"-com"),css(E,"-comfix"),hash.del("com")),l&&setTimeout(function(){e(g,0)},550)}function R(a){var b="",c=A[a];c&&c.length?
		(c.forEach(function(c){b+="<li><header>"+c.who+' <span title="'+c.when.replace("T"," ")+'">'+loc.reldate(c.when)+"</span></header><content>"+c.what.replace("\n","<br/>")+"</content>"+(c.own?'<button class="del" onclick="walli.rmcom(\''+a.replace("'","\\'")+"',"+c.id+')">'+loc.bdel+"</button>":"")+"</li>"}),_(H,b),H.scrollTop=H.scrollHeight,_("#comcount",999<c.length?Math.floor(c.length/1E3)+"K+":c.length)):(_(H,loc.nocom),_("#comcount","0"))}function B(a){a&&a.stopPropagation&&a.stopPropagation()}
		function O(){l?hash.set("f",k[m]):q?hash.set("f",q):hash.del("f")}function U(){var a=hash.get("f"),b=hash.get("com"),c=/^(.+\/)([^\/]*)$/.test(a)?RegExp.$1:"/",d=RegExp.$2;b?Q(!0,!0):K&&Q(!1,!0);return a.length?(b=function(){var b=k.indexOf(a),c=0;l&&b!=m&&(c=b<m?-1:1);-1!=b?walli.show(b,c):t("thumb")},c!=q?n(c,b):d?b():t("thumb"),!0):!1}var W=5,D=!1,z=!1,M,q,k=[],N=[],u=[],m=!1,g=0,j=[],P=[],E,I,F,L=0,J=!1,l,p,x={},A=[],K,G,H,V,w=!1,C=!0;return{setup:function(a){V=a.comments;M=a.refresh;w=a.god;
		C=a.zip;E=_("#view");H=_("#coms");p=_("#slide");p.onmousewheel=function(a){l&&(0>(a.wheelDelta||a.detail/3)?walli.prev():walli.next(),a.preventDefault())};var b=function(){j[g].className="animated";j[g].style.left=x.l+"px";x={}};p.onmousedown=p.ontouchstart=function(a){a.preventDefault();a.touches&&(a=a.touches[0]);j[g].className="touch";x={d:!0,x:a.pageX,l:parseInt(j[g].style.left,10),h:setTimeout(b,1E3)}};p.onmousemove=p.ontouchmove=function(a){x.d&&(a.preventDefault(),a.touches&&(a=a.touches[0]),
		a=a.pageX-x.x,j[g].style.left=x.l+a+"px",80<Math.abs(a)&&(clearTimeout(x.h),x={},j[g].className="animated",80<a?walli.prev():walli.next()))};p.onmouseup=p.onmouseout=p.ontouchend=p.ontouchcancel=function(a){a.preventDefault();x.d&&(clearTimeout(x.h),b())};j=[_("#img0"),_("#img1")];p.ondragstart=j[0].ondragstart=j[1].ondragstart=function(a){a.preventDefault();return!1};window.onresize=function(){l&&(j[1-g].className="",e(1-g,1),e(g,0));f()};window.onorientationchange=function(){var a=_("#viewport"),
		b=window.orientation||0;a.setAttribute("content",90==b||-90==b||270==b?"height=device-width,width=device-height,initial-scale=1.0,maximum-scale=1.0":"height=device-height,width=device-width,initial-scale=1.0,maximum-scale=1.0");l&&e(g,0)};hotkeys.add("CTRL+D",function(){css("#log","*active")},!0).add("SPACE",walli.toggleplay).add("C",walli.togglecom).add("HOME",walli.first).add("LEFT",walli.prev).add("RIGHT",walli.next).add("END",walli.last).add(["ESC","UP"],walli.back).add("DOWN",function(){!l&&
		k.length&&walli.show(0)});_("#bprev").onclick=walli.prev;_("#bnext").onclick=walli.next;_("#bplay").onclick=walli.toggleplay;_("#bthumb").onclick=walli.thumb;_("#bzip").onclick=walli.dlzip;C||(_("#bzip").className="hide");V&&(I=_("#who"),F=_("#what"),F.onfocus=I.onfocus=function(){y(!1)},_("#comments").onclick=B,_("#bcom").onclick=walli.togglecom,_("#bsend").onclick=walli.sendcom);w?(_("#blogout").onclick=walli.logout,FormData?(_("#iupload").onchange=function(){for(var a=new FormData,b=0,c=this.files,
		d,h=0;h<c.length;++h)d=c[h],b+=d.size,a.append("file"+h,d);if(confirm(loc.tpl("uploadfiles",{z:loc.size(b),nb:c.length}))){var e=new XMLHttpRequest;e.open("POST","walli.php?!=img&&path="+q);e.onload=function(){if(200==e.status){var a=JSON.parse(e.responseText);osd.loc("uploaded",{nb:a.added});a.added&&n(q)}else osd.error("error "+e.status)};e.upload.onprogress=function(a){event.lengthComputable&&osd.set(a.loaded,a.total)};e.send(a)}},_("#bupload").onclick=function(){_("#iupload").click()}):css("#bupload",
		{diplay:"none"}),_("#bdel").onclick=walli.del,_("#bdiag").onclick=walli.switchdiag,_("#bflush").onclick=walli.flush):a.admin&&(_("#blogin").onclick=walli.login);log.info("show on!");var c=_("#intro");if(c){var d=setTimeout(function(){css(c,"hide")},5E3);c.onclick=function(){clearTimeout(d);css(c,"hide")}}U()||(t("thumb"),n("/"));hash.onchange(U)},login:function(){document.location="?login"+document.location.hash},logout:function(){document.location="?logout"+document.location.hash},del:function(){if(w){var a=
		k.filter(function(a,b){return-1!=u.indexOf(b)});u.length?ajax({type:"POST",url:"?!=del",data:{files:a.join("*")},ok:function(a){osd.loc("deleted",{nb:a.deleted});a.deleted&&n(q)},error:osd.error}):osd.error(loc.noselection)}},switchdiag:function(){var a=_("#diag");a?document.body.removeChild(a):walli.diag()},diag:function(){w&&ajax({url:"?!=diag",data:{path:q},ok:function(a){var b=_("#diag"),c="<ul>",d;for(d in a.stats)c+='<li class="stat">'+("size"==d?loc.size(a.stats[d]):a.stats[d]+" "+d)+"</li>";
		for(d in a.checks)c+='<li class="'+(a.checks[d]?"ok":"bad")+'">'+d+(a.checks[d]?" enabled":" disabled")+"</li>";b||(b=document.createElement("div"),b.id="diag",b.onclick=function(){document.body.removeChild(b)},document.body.appendChild(b));b.innerHTML=c},error:osd.error})},flush:function(){w&&ajax({url:"?!=flush",ok:function(a){osd.loc("flushed",{nb:a.flushed})},error:osd.error})},dlzip:function(){var a=u.length?k.filter(function(a,b){return-1!=u.indexOf(b)}):k;C&&a.length?(_("#bzip",loc.zip),ajax({type:"POST",
