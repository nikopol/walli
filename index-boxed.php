...
<?php } ?>
	...
	<script>
		...
		append(h,'<span class="minicom">'+g(G[a].length)+"</span>");diapos.appendChild(h);"dir"==c||0==f&&0==m?(c=2,p=1<b?2:1):1<b-f&&.5<l&&!s(f,m,2,1)?(c=.8<l&&n>b?2:1,p=2):(c=.7<l&&n>b?2:1,p=1);t(h,f,m,p,c);for(k.src="?!=mini&file="+encodeURIComponent(a)+"&w="+q[p]+"&h="+q[c];s(f,m,1,1);)f++,f>=b&&(f=0,m++);n--};_("#diapos","");osd.start(h.length+I.length);I.forEach(function(a){u(a,function(){l(a)},"dir")});h.forEach(function(a,d){setTimeout(function(){u(a,function(){walli.show(d,0)},"",d)},10*d)})}},build:function(){v.lastnc=
		0;this.make()},resize:function(){this.make()},rotate:function(){this.make()}},"default":{build:function(){var a=function(a,p,c,q){var b=function(){var c=document.createElement("img");c.onload=function(){osd.inc();log.debug(a+" loaded");css(this.parentNode,"+loaded")};c.onclick=p;return c}(),f=document.createElement("li");css(f,"diapo "+c);f.appendChild(b);f.setAttribute("title",e(a));void 0!=q&&(b.id="diapo"+q,(H||y)&&append(f,'<input type="checkbox" id="chk'+q+'" n="'+q+'" onchange="walli.zwap('+
		q+')"/><label for="chk'+q+'"></label>'));(G[a]||[]).length&&append(f,'<span class="minicom">'+g(G[a].length)+"</span>");diapos.appendChild(f);b.src="?!=mini&file="+encodeURIComponent(a)+"&w="+v.size+"&h="+v.size};osd.start(h.length+I.length);I.forEach(function(d){a(d,function(){l(d)},"dir")});h.forEach(function(d,b){setTimeout(function(){a(d,function(){walli.show(b,0)},"",b)},b)})},resize:function(){},rotate:function(){}}};return{setup:function(a){ja=a.comments;V=a.refresh;y=a.god;H=a.zip;v=a.thumbnail||
		...
		t.ondragstart=k[0].ondragstart=k[1].ondragstart=function(a){a.preventDefault();return!1};window.onresize=function(){r&&(k[1-f].className="",x(1-f,1),x(f,0));m();v.layout.resize()};window.onorientationchange=function(){var a=_("#viewport"),d=window.orientation||0;a.setAttribute("content",90==d||-90==d||270==d?"height=device-width,width=device-height,initial-scale=1.0,maximum-scale=1.0":"height=device-height,width=device-width,initial-scale=1.0,maximum-scale=1.0");r&&x(f,0);v.layout.rotate()};hotkeys.add("CTRL+D",
		...
		function(){Q(-1)});_("#bprev").onclick=walli.prev;_("#bnext").onclick=walli.next;_("#bplay").onclick=walli.toggleplay;_("#bthumb").onclick=walli.thumb;_("#bzip").onclick=walli.dlzip;H||(_("#bzip").className="hide");ja&&(P=_("#who"),N=_("#what"),N.onfocus=P.onfocus=function(){Y(!1)},_("#comments").onclick=z,_("#bcom").onclick=walli.togglecom,_("#bsend").onclick=walli.sendcom);y?(_("#blogout").onclick=walli.logout,FormData?(_("#iupload").onchange=function(a){a=new FormData;for(var d=0,c=this.files,
		...
		A.length?h.filter(function(a,b){return-1!=A.indexOf(b)}):h;H&&a.length?(_("#bzip",loc.zip),ajax({type:"POST",url:"?!=zip",data:{files:a.join("*")},ok:function(a){document.location="?!=zip&zip="+a.zip;walli.zwap()},error:osd.error})):osd.loc("nozip")},zwap:function(a){void 0!=a&&(-1==A.indexOf(a)?A.push(a):A=A.filter(function(d){return d!=a}));A.length?(b("selected",loc.tpl("dlsel",{nb:A.length})),y&&(_("#bdel").className="")):(b("all",loc.dlall),y&&(_("#bdel").className="hide"))},thumb:function(){E("thumb")},
		show:function(a,d,b){h.length&&(n=0>a?h.length+a:a>=h.length?a%h.length:a,r||E("tof"),css("#mask","+active"),w(n,function(a,b){css("#mask","-active");r?f=1-f:d=0;X[f]={w:b.width,h:b.height};t.removeChild(k[f]);k[f].src=b.src;if(d)"box"==ba?(css(k[f],0>d?"left":"right"),x(f,0),t.appendChild(k[f]),x(f,0),css(k[f],"animated center"),css(k[1-f],"animated "+(0<d?"left":"right")),x(1-f,0)):(css(k[f],0>d?"left":"right"),x(f,d),t.appendChild(k[f]),x(f,0),css(k[f],"animated center"),css(k[1-f],"animated "+
		(0<d?"left":"right")),x(1-f,-d));else{css(k[f],"");var e=position("#diapo"+n);css(k[f],{width:e.width+"px",height:e.height+"px",left:e.left+"px",top:e.top+"px"});t.appendChild(k[f]);css(k[f],"animated");x(f,0)}ca();W();ia();walli.exif();1<h.length&&w((n+1)%h.length,function(){})}),aa(h[n]))},next:function(a){z(a);r?walli.show(n+1,1):C(1,0)},prev:function(a){z(a);r?walli.show(n-1,-1):C(-1,0)},down:function(a){z(a);r?walli.show(n+1,1):C(0,1)},up:function(a){z(a);r?walli.show(n-1,-1):C(0,-1)},pgdown:function(a){z(a);
		r?walli.show(n+5,1):C(0,5)},pgup:function(a){z(a);r?walli.show(n-5,-1):C(0,-5)},first:function(a){z(a);r?walli.show(0,-1):C(0,0,!0)},last:function(a){z(a);r?walli.show(-1,1):C(-1,-1,!0)},play:function(a){z(a);h.length&&(r||walli.show(n,0),E("tof"))},stop:function(a){z(a);E("thumb")},toggleplay:function(a){z(a);r?Y(!S):(J[s]||C(0,0,!0),J[s].o&&J[s].o.children[0].click(a))},togglecom:function(a){a&&a.stopPropagation();$(!T)},sendcom:function(){1>P.value.length?(osd.loc("emptywho"),P.focus()):1>N.value.length?
