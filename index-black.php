...
<?php } ?>
	...
	<script>
		...
		"g"),1<c.nb?"s":""));return l};if(loc.title)for(b in loc.title)(t=_("#"+b))&&t.setAttribute("title",loc.title[b]);if(loc.holder)for(b in loc.holder)(t=_("#"+b))&&t.setAttribute("placeholder",loc.holder[b]);if(loc.text)for(b in loc.text)_("#"+b,loc.text[b]);c="<table><tr><th>"+loc.help.th.join("</th><th>")+"</th></tr>";for(b in loc.help.tr)c+="<tr><td>"+loc.help.tr[b].join("</td><td>")+"</td></tr>";_("#help",c)};ready(function(){setlocale(hash.get("lang")||navigator.language)});
		var log=function(){var c,b,t=(new Date).getTime(),d={debug:1,info:2,warn:3,error:4},u=hash.get("log"),l=u&&d[u]?d[u]:0;if(!l)return{debug:function(){},info:function(){},warn:function(){},error:function(){}};console.log?c=function(b,c){if(d[b]>=l){var C,t=("     "+b).substr(-5)+"|";for(C in c)console.log(t+c[C])}}:(ready(function(){append(document.body,'<div id="log"></div>');b=_("#log")}),c=function(c,M){if(b&&d[c]>=l){var C,u=("000000"+((new Date).getTime()-t)).substr(-6);for(C in M)append(b,'<div class="'+
		c+'"><span class="timer">'+u+"</span>"+M[C].replace(" ","&nbsp;")+"</div>");b.scrollTop=b.scrollHeight}});return{debug:function(){c("debug",arguments)},info:function(){c("info",arguments)},warn:function(){c("warn",arguments)},error:function(){c("error",arguments)}}}(),osd;
		osd=function(){var c,b,t,d,u,l,g=!1;ready(function(){c=_("#osd");b=_("#progress")});return{hide:function(){g&&(g=clearTimeout(g));css(c,"-active")},show:function(){css(c,"+active")},error:function(b){log.error(b);osd.info(b,"error",5E3)},info:function(b,d,l){_(c,b).className=d||"";g&&clearTimeout(g);osd.show();g=setTimeout(osd.hide,l||3E3)},loc:function(b,d){osd.info(loc.tpl(b,d))},start:function(c,g,u){t=u||"%v/%m";d=c;l=g;d&&(css(b,"+active"),osd.set(0))},set:function(c,g){g&&g!=d&&(d=g,css(b,"+active"));
		u=c;_("#progresstext",t.replace(/%v/,c).replace(/%m/,d));c>=d&&(css(b,"-active"),l&&l());_("#progressbar").style.width=d?Math.floor(position("#progress").width*c/d)+"px":0},inc:function(){osd.set(++u)}}}();var walli;
		walli=function(){function c(){var a=position("#thumbbar");_("#diapos").style.top=a.top+a.height+"px"}function b(a,s){G&&(_("#bzip",s).className=a,c())}function t(a,s){var e=m[a],b,r=Date.now();b=new Image;S++;b.onload=function(b){b||(b=window.event);log.info("image #"+a+" "+e+" loaded in "+(Date.now()-r)/1E3+"s");s(a,b.target||b.srcElement);S--};b.onerror=function(){log.error("error loading "+("image #"+a+" "+e));s(a,null);S--};b.src=encodeURIComponent(e)}function d(a){a=/([^\/]+)\/$/.test(a)?RegExp.$1:
		a.replace(/^.*\//g,"");return a.replace(/\.[^\.]*$/,"").replace(/[\._\|]/g," ")}function u(){T&&!D&&(D=setInterval(function(){log.debug("refresh required");ajax("?!=count&path="+q,function(a){U.length==a.dirs&&m.length==a.files||l(q)})},1E3*T))}function l(a,s){D&&(clearInterval(D),D=!1);log.debug("loading path "+(a||"/"));b("hide",loc.dlall);w&&(_("#bdel").className="hide");x=[];ajax("?!=ls&path="+a,function(a){var H=_("#diapos","");q=a.path;log.info((q||"/")+"loaded with "+a.dirs.length+" subdirs and "+
		a.files.length+" files found");if(q.length){var r=q.replace(/[^\/]+\/$/,"/"),P=document.createElement("li");css(P,"diapo up loaded");P.setAttribute("title",loc.updir);P.onclick=function(){l(r)};H.appendChild(P);var h="",g="";q.split("/").forEach(function(a){a&&(h+=a+"/",g+="<button onclick=\"walli.cd('"+h+"')\">"+a+"</button>")});_("#path",g);c()}else _("#path","");var n=function(a,b,s,r){var e=function(){var s=document.createElement("img");s.onload=function(){osd.inc();log.debug(a+" loaded");css(this.parentNode,
		"+loaded")};s.onclick=b;return s}(),c=document.createElement("li");css(c,"diapo "+s);c.appendChild(e);c.setAttribute("title",d(a));void 0!=r&&(e.id="diapo"+r,(G||w)&&append(c,'<input type="checkbox" id="chk'+r+'" n="'+r+'" onchange="walli.zwap('+r+')"/><label for="chk'+r+'"></label>'));(E[a]||[]).length&&append(c,'<span class="minicom">'+(999<E[a].length?Math.floor(E[a].length/1E3)+"K+":E[a].length)+"</span>");H.appendChild(c);e.src="?!=mini&file="+encodeURIComponent(a)};m=a.files;U=a.dirs;E=a.coms;
		s&&s();osd.start(m.length+U.length);a.dirs.forEach(function(a){n(a,function(){l(a)},"dir")});a.files.forEach(function(a,b){setTimeout(function(){n(a,function(){walli.show(b,0)},"",b)},b)});a.files.length&&G&&b("all");V();u();w&&_("#diag")&&walli.diag();F[q]&&y(0,0)})}function g(a,b){if(W[a]){var e=p.clientWidth,H=p.clientHeight,r=W[a],c=r.h,d=r.w;d>e&&(d=e,c=Math.floor(d*(r.h/r.w)));c>H&&(c=H,d=Math.floor(c*(r.w/r.h)));css(f[a],{width:d+"px",height:c+"px",left:Math.floor((e-d)/2+2*e*b)+"px",top:Math.floor((H-
		c)/2)+"px"})}}function M(){Q&&(I&&clearTimeout(I),I=setTimeout(walli.next,1E3*J))}function C(a){J+=a;0>J&&(J=0);osd.loc("delay",{s:J})}function aa(){css(document.body,"-sleep");document.body.onmousemove=document.body.ontouchstart=function(a){if(a.x!=ba||a.y!=ca)ba=a.x,ca=a.y,aa()};A&&clearTimeout(A);A=setTimeout(ha,1E3*ia)}function ha(){A&&(A=clearTimeout(A));css(document.body,"+sleep")}function X(a){Q!==a&&((Q=a)?(M(),a=document.documentElement,a.requestFullscreen?a.requestFullscreen():a.mozRequestFullScreen?
		a.mozRequestFullScreen():a.webkitRequestFullScreen&&a.webkitRequestFullScreen(),css("#bplay","+active"),css("#view","+play"),osd.loc("play"),aa()):(I&&(I=clearTimeout(I)),A&&(A=clearTimeout(A)),document.body.onmousemove=document.body.ontouchstart=!1,css(document.body,"-sleep"),css("#bplay","-active"),css("#view","-play"),osd.loc("stop")))}function B(a){Y!==a&&(Y=a,log.debug("switch to "+a+" mode"),n=!0,"tof"==Y?(D&&(clearInterval(D),D=!1),css(K,"+active"),css("#thumb","-active")):(n=!1,X(!1),osd.hide(),
		css(f[0],""),css(f[1],""),css(K,"-active"),css("#exif","-active"),css("#thumb","+active"),u()),V())}function da(a){var b=_("#diapo"+k).parentNode,e=_("#minicom"+k);e&&b.removeChild(e);0<a&&append(b,'<span id="minicom'+k+'" class="minicom">'+a+"</span>")}function Z(a,b){if(b||n&&R!==a)R=a,n&&(css(f[1-h],""),g(1-h,2)),a?(css("#bcom","+active"),css(K,"+com"+(b?"fix":"")),hash.set("com",1)):(css("#bcom","-active"),css(K,"-com"),css(K,"-comfix"),hash.del("com")),n&&setTimeout(function(){g(h,0)},550)}function $(a){var b=
		"",e=E[a];e&&e.length?(e.forEach(function(e,r){b+="<li><header>"+e.who+' <span title="'+e.when.replace("T"," ")+'">'+loc.reldate(e.when)+"</span></header><content>"+e.what.replace("\n","<br/>")+"</content>"+(e.own?'<button class="del" onclick="walli.rmcom(\''+a.replace("'","\\'")+"',"+e.id+')">'+loc.bdel+"</button>":"")+"</li>"}),_(N,b),N.scrollTop=N.scrollHeight,_("#comcount",999<e.length?Math.floor(e.length/1E3)+"K+":e.length)):(_(N,loc.nocom),_("#comcount","0"))}function ja(a,b){ajax({type:"POST",
		url:"?!=comment",data:{file:m[k],who:a,what:b},ok:function(a){E[a.file]=a.coms;$(m[k]);da(a.coms.length);L.value=""},error:osd.error})}function ka(a,b){ajax({type:"POST",url:"?!=uncomment",data:{file:a,id:b},ok:function(a){E[a.file]=a.coms;$(m[k]);da(a.coms.length)},error:osd.error})}function v(a){a&&a.stopPropagation&&a.stopPropagation()}function V(){n?hash.set("f",m[k]):q?hash.set("f",q):hash.del("f")}function ea(){var a=hash.get("f"),b=hash.get("com"),e=/^(.+\/)([^\/]*)$/.test(a)?RegExp.$1:"/",
		c=RegExp.$2;b?Z(!0,!0):R&&Z(!1,!0);return a.length?(b=function(){var b=m.indexOf(a),e=0;n&&b!=k&&(e=b<k?-1:1);-1!=b?walli.show(b,e):B("thumb")},e!=q?l(e,b):c?b():B("thumb"),!0):!1}function y(a,b,e){if(!n){var c=F[q]?F[q]:{x:0,y:0,n:0};e&&(c.x=a,c.y=b);c.o&&css(c.o,"-cursor");var r=_("#diapos li"),d=_("#diapos"),h=d.getBoundingClientRect(),g=[],l=!1,m=-1,f,k,p;if(r.length){for(;++m<r.length;)if(f=r[m].getBoundingClientRect(),f={t:f.top-h.top+d.scrollTop,w:f.width,h:f.height},f.w){k=Math.round(f.t+
		f.h/2);if(!1===l||k>l)l=k,g.push([]),p=g.length-1;g[p].push({t:f.t,b:f.t+f.h,n:m});e||c.n!=m||(c={x:a+g[p].length-1,y:b+p,n:m})}c.y>p?c.y=0:0>c.y&&(c.y=p);c.x>=g[c.y].length?c.x=0:0>c.x&&(c.x=g[c.y].length-1);k=g[c.y][c.x];c.o=r[k.n];c.n=k.n;f={t:d.scrollTop,b:d.scrollTop+h.height,h:h.height};p=f.t;k.b>f.b?p=k.b-f.h+30:k.t<f.t&&(p=k.t-30);d.scrollTop=p;css(c.o,"+cursor")}F[q]=c}}function fa(){n&&osd.info(d(m[k])+" <sup>"+(k+1)+"/"+m.length+"</sup>")}var J=5,ia=3,I=!1,D=!1,A=!1,T,q,m=[],U=[],x=[],
		k=!1,h=0,f=[],W=[],K,O,L,S=0,Q=!1,n,p,z={},E=[],R,Y,N,ga,w=!1,G=!0,ba=0,ca=0,F={};return{setup:function(a){ga=a.comments;T=a.refresh;w=a.god;G=a.zip;K=_("#view");N=_("#coms");p=_("#slide");p.onmousewheel=function(a){n&&(0>(a.wheelDelta||a.detail/3)?walli.prev():walli.next(),a.preventDefault())};var b=function(){f[h].className="animated";f[h].style.left=z.l+"px";z={}};p.onmousedown=p.ontouchstart=function(a){a.preventDefault();a.touches&&(a=a.touches[0]);f[h].className="touch";z={d:!0,x:a.pageX,l:parseInt(f[h].style.left,
		10),h:setTimeout(b,1E3)}};p.onmousemove=p.ontouchmove=function(a){z.d&&(a.preventDefault(),a.touches&&(a=a.touches[0]),a=a.pageX-z.x,f[h].style.left=z.l+a+"px",80<Math.abs(a)&&(clearTimeout(z.h),z={},f[h].className="animated",80<a?walli.prev():walli.next()))};p.onmouseup=p.onmouseout=p.ontouchend=p.ontouchcancel=function(a){a.preventDefault();z.d&&(clearTimeout(z.h),b())};f=[_("#img0"),_("#img1")];p.ondragstart=f[0].ondragstart=f[1].ondragstart=function(a){a.preventDefault();return!1};window.onresize=
		...
		walli.prev).add("RIGHT",walli.next).add("UP",walli.up).add("DOWN",walli.down).add("PAGEUP",walli.pgup).add("PAGEDOWN",walli.pgdown).add("END",walli.last).add(["ESC","BACKSPACE"],walli.back).add("DOWN",function(){!n&&m.length&&walli.show(0)}).add(["?","H"],function(){css("#help","*active")}).add(["T"],fa).add(["I"],walli.togglexif).add(["+"],function(){C(1)}).add(["-"],function(){C(-1)});_("#bprev").onclick=walli.prev;_("#bnext").onclick=walli.next;_("#bplay").onclick=walli.toggleplay;_("#bthumb").onclick=
		walli.thumb;_("#bzip").onclick=walli.dlzip;G||(_("#bzip").className="hide");ga&&(O=_("#who"),L=_("#what"),L.onfocus=O.onfocus=function(){X(!1)},_("#comments").onclick=v,_("#bcom").onclick=walli.togglecom,_("#bsend").onclick=walli.sendcom);w?(_("#blogout").onclick=walli.logout,FormData?(_("#iupload").onchange=function(a){a=new FormData;for(var b=0,c=this.files,e,d=0;d<c.length;++d)e=c[d],b+=e.size,a.append("file"+d,e);if(confirm(loc.tpl("uploadfiles",{z:loc.size(b),nb:c.length}))){var s=new XMLHttpRequest;
		s.open("POST","?!=img&path="+q);s.onload=function(){if(200==s.status){var a=JSON.parse(s.responseText);osd.loc("uploaded",{nb:a.added});a.added&&l(q)}else osd.error("error "+s.status)};s.upload.onprogress=function(a){event.lengthComputable&&osd.set(a.loaded,a.total)};s.send(a)}},_("#bupload").onclick=function(){_("#iupload").click()}):css("#bupload",{diplay:"none"}),_("#bdel").onclick=walli.del,_("#bdiag").onclick=walli.togglediag,_("#bflush").onclick=walli.flush,_("#bmkdir").onclick=walli.mkdir):
		a.admin&&(_("#blogin").onclick=walli.login);log.info("show on!");var e=_("#intro");if(e){var d=setTimeout(function(){css(e,"hide")},5E3);e.onclick=function(){clearTimeout(d);css(e,"hide")}}ea()||(B("thumb"),l("/"));hash.onchange(ea)},login:function(){document.location="?login"+document.location.hash},logout:function(){document.location="?logout"+document.location.hash},del:function(){if(w){var a=m.filter(function(a,b){return-1!=x.indexOf(b)});x.length?ajax({type:"POST",url:"?!=del",data:{files:a.join("*")},
		ok:function(a){osd.loc("deleted",{nb:a.deleted});a.deleted&&l(q)},error:osd.error}):osd.error(loc.noselection)}},togglexif:function(){css("#exif","*active");walli.exif()},exif:function(){var a=_("#exif.active");a.length&&(_(a,loc.load),ajax({url:"?!=exif&file="+m[k],ok:function(b){var c,d,f;if(b&&b.exif)for(d in c="",b.exif){c+="<div><h3>"+d+"</h3><table>";for(f in b.exif[d])null!=b.exif[d][f]&&""!=b.exif[d][f]&&(c+="<tr><th>"+f+"</th><td>"+b.exif[d][f]+"</td></tr>");c+="</table></div>"}else c=loc.exifnotfound;
		_(a,c)},error:osd.error}))},togglediag:function(){var a=_("#diag");a?document.body.removeChild(a):walli.diag()},diag:function(){w&&ajax({url:"?!=diag",data:{path:q},ok:function(a){var b=_("#diag"),c="<ul>",d;for(d in a.stats)c+='<li class="stat">'+("size"==d?loc.size(a.stats[d]):a.stats[d]+" "+d)+"</li>";for(d in a.checks)c+='<li class="'+(a.checks[d]?"ok":"bad")+'">'+d+(a.checks[d]?" enabled":" disabled")+"</li>";b||(b=document.createElement("div"),b.id="diag",b.onclick=function(){document.body.removeChild(b)},
		document.body.appendChild(b));b.innerHTML=c},error:osd.error})},flush:function(){w&&ajax({url:"?!=flush",ok:function(a){osd.loc("flushed",{nb:a.flushed})},error:osd.error})},mkdir:function(){var a;w&&(a=prompt(loc.mkdir))&&ajax({type:"POST",url:"?!=mkdir",data:{dir:a,path:q},ok:function(){l(q)},error:osd.error})},dlzip:function(){var a=x.length?m.filter(function(a,b){return-1!=x.indexOf(b)}):m;G&&a.length?(_("#bzip",loc.zip),ajax({type:"POST",url:"?!=zip",data:{files:a.join("*")},ok:function(a){document.location=
		"?!=zip&zip="+a.zip;walli.zwap()},error:osd.error})):osd.loc("nozip")},zwap:function(a){void 0!=a&&(-1==x.indexOf(a)?x.push(a):x=x.filter(function(b){return b!=a}));x.length?(b("selected",loc.tpl("dlsel",{nb:x.length})),w&&(_("#bdel").className="")):(b("all",loc.dlall),w&&(_("#bdel").className="hide"))},thumb:function(){B("thumb")},show:function(a,b,c){m.length&&(k=0>a?m.length+a:a>=m.length?a%m.length:a,n||B("tof"),css("#mask","+active"),t(k,function(a,c){css("#mask","-active");n?h=1-h:b=0;W[h]=
		{w:c.width,h:c.height};p.removeChild(f[h]);f[h].src=c.src;if(b)css(f[h],0>b?"left":"right"),g(h,b),p.appendChild(f[h]),g(h,0),css(f[h],"animated center"),css(f[1-h],"animated "+(0<b?"left":"right")),g(1-h,-b);else{css(f[h],"");var d=position("#diapo"+k);css(f[h],{width:d.width+"px",height:d.height+"px",left:d.left+"px",top:d.top+"px"});p.appendChild(f[h]);css(f[h],"animated");g(h,0)}M();V();fa();walli.exif();1<m.length&&t((k+1)%m.length,function(){})}),$(m[k]))},next:function(a){v(a);n?walli.show(k+
		1,1):y(1,0)},prev:function(a){v(a);n?walli.show(k-1,-1):y(-1,0)},down:function(a){v(a);n?walli.show(k+1,1):y(0,1)},up:function(a){v(a);n?walli.show(k-1,-1):y(0,-1)},pgdown:function(a){v(a);n?walli.show(k+5,1):y(0,5)},pgup:function(a){v(a);n?walli.show(k-5,-1):y(0,-5)},first:function(a){v(a);n?walli.show(0,-1):y(0,0,!0)},last:function(a){v(a);n?walli.show(-1,1):y(-1,-1,!0)},play:function(a){v(a);m.length&&(n||walli.show(k,0),B("tof"))},stop:function(a){v(a);B("thumb")},toggleplay:function(a){v(a);
		n?X(!Q):(F[q]||y(0,0,!0),F[q].o&&F[q].o.children[0].click(a))},togglecom:function(a){a&&a.stopPropagation();Z(!R)},sendcom:function(){1>O.value.length?(osd.loc("emptywho"),O.focus()):1>L.value.length?(osd.loc("emptywhat"),L.focus()):ja(O.value,L.value)},rmcom:function(a,b){ka(a,b)},back:function(){if(n)return B("thumb");var a=q.split("/");1<a.length&&l(a.slice(0,a.length-2).join("/"))},cd:function(a){B("thumb");l(a)}}}();
	</script>
	<script>
		ready(function(){
			walli.setup({
