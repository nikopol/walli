...
<?php } ?>
	...
	<script>
		...
		"","change le d\u00e9lai du diaporama"]]},bdel:"&#10006;",nocom:"soyez le premier \u00e0 laisser un commentaire",emptywho:"de la part de ?",emptywhat:"dites quelque chose\u2026",play:"&#9654; LECTURE",stop:"&#9632; STOP",dlall:"tout t\u00e9l\u00e9charger",dlsel:"t\u00e9l\u00e9charger la s\u00e9lection",zip:"compression\u2026",nozip:"rien \u00e0 t\u00e9l\u00e9charger",updir:"",uploadfiles:"poster %nb image%s (%z octets) ?",flushed:"%nb fichier%s supprim\u00e9%s",uploaded:"%nb image%s ajout\u00e9e%s",
		deleted:"%nb image%s effac\u00e9e%s",mkdir:"nom du dossier ?",delay:"le diaporama passe \u00e0 %ss"}},loc,setlocale=function(c){var b,s;loc=locales[c]?locales[c]:locales.en;loc.reldate=function(d){var b=(new Date).getTime();d=(new Date(d)).getTime();b=(b-d)/1E3;d=function(b,d){d=Math.round(d);return b.replace("%d",d).replace("%s",1<d?"s":"")};return 60>b?loc.date.now:3600>b?d(loc.date.min,b/60):86400>b?d(loc.date.hour,b/3600):172800>b?loc.date.yesterday:604800>b?d(loc.date.day,b/86400):2592E3>b?d(loc.date.week,
		b/604800):d(loc.date.month,b/2592E3)};loc.size=function(b){return 2048>b?b+"b":1E6>b?Math.round(b/1024)+"kb":1E9>b?Math.round(b/1E6)+"M":Math.round(b/1E9)+"G"};loc.tpl=function(b,c){var k=loc[b],g;for(g in c)k=k.replace(RegExp("%"+g,"g"),c[g]),"nb"==g&&(k=k.replace(RegExp("%s","g"),1<c.nb?"s":""));return k};if(loc.title)for(b in loc.title)(s=_("#"+b))&&s.setAttribute("title",loc.title[b]);if(loc.holder)for(b in loc.holder)(s=_("#"+b))&&s.setAttribute("placeholder",loc.holder[b]);if(loc.text)for(b in loc.text)_("#"+
		b,loc.text[b]);c="<table><tr><th>"+loc.help.th.join("</th><th>")+"</th></tr>";for(b in loc.help.tr)c+="<tr><td>"+loc.help.tr[b].join("</td><td>")+"</td></tr>";_("#help",c)};ready(function(){setlocale(hash.get("lang")||navigator.language)});
		var log=function(){var c,b,s=(new Date).getTime(),d={debug:1,info:2,warn:3,error:4},t=hash.get("log"),k=t&&d[t]?d[t]:0;if(!k)return{debug:function(){},info:function(){},warn:function(){},error:function(){}};console.log?c=function(b,c){if(d[b]>=k){var B,s=("     "+b).substr(-5)+"|";for(B in c)console.log(s+c[B])}}:(ready(function(){append(document.body,'<div id="log"></div>');b=_("#log")}),c=function(c,K){if(b&&d[c]>=k){var B,t=("000000"+((new Date).getTime()-s)).substr(-6);for(B in K)append(b,'<div class="'+
		c+'"><span class="timer">'+t+"</span>"+K[B].replace(" ","&nbsp;")+"</div>");b.scrollTop=b.scrollHeight}});return{debug:function(){c("debug",arguments)},info:function(){c("info",arguments)},warn:function(){c("warn",arguments)},error:function(){c("error",arguments)}}}(),osd;
		osd=function(){var c,b,s,d,t,k,g=!1;ready(function(){c=_("#osd");b=_("#progress")});return{hide:function(){g&&clearTimeout(g);g=!1;css(c,"-active")},show:function(){css(c,"+active")},error:function(b){log.error(b);osd.info(b,"error",5E3)},info:function(b,d,k){_(c,b).className=d||"";g&&clearTimeout(g);osd.show();g=setTimeout(osd.hide,k||3E3)},loc:function(b,d){osd.info(loc.tpl(b,d))},start:function(c,g,t){s=t||"%v/%m";d=c;k=g;d&&(css(b,"+active"),osd.set(0))},set:function(c,g){g&&g!=d&&(d=g,css(b,
		"+active"));t=c;_("#progresstext",s.replace(/%v/,c).replace(/%m/,d));c>=d&&(css(b,"-active"),k&&k());_("#progressbar").style.width=d?Math.floor(position("#progress").width*c/d)+"px":0},inc:function(){osd.set(++t)}}}();var walli;
		walli=function(){function c(){var a=position("#thumbbar");_("#diapos").style.top=a.top+a.height+"px"}function b(a,b){F&&(_("#bzip",b).className=a,c())}function s(a,b){var u=e[a],f,p=Date.now();f=new Image;S++;f.onload=function(f){f||(f=window.event);log.info("image #"+a+" "+u+" loaded in "+(Date.now()-p)/1E3+"s");b(a,f.target||f.srcElement);S--};f.onerror=function(){log.error("error loading "+("image #"+a+" "+u));b(a,null);S--};f.src=encodeURIComponent(u)}function d(a){a=/([^\/]+)\/$/.test(a)?RegExp.$1:
		a.replace(/^.*\//g,"");return a.replace(/\.[^\.]*$/,"").replace(/[\._\|]/g," ")}function t(){T&&!C&&(C=setInterval(function(){log.debug("refresh required");ajax("?!=count&path="+r,function(a){U.length==a.dirs&&e.length==a.files||k(r)})},1E3*T))}function k(a,N){C&&(clearInterval(C),C=!1);log.debug("loading path "+(a||"/"));b("hide",loc.dlall);w&&(_("#bdel").className="hide");x=[];ajax("?!=ls&path="+a,function(a){var f=_("#diapos","");r=a.path;log.info((r||"/")+"loaded with "+a.dirs.length+" subdirs and "+
		a.files.length+" files found");if(r.length){var p=r.replace(/[^\/]+\/$/,"/"),O=document.createElement("li");css(O,"diapo up loaded");O.setAttribute("title",loc.updir);O.onclick=function(){k(p)};f.appendChild(O);var h="",g="";r.split("/").forEach(function(a){a&&(h+=a+"/",g+="<button onclick=\"walli.cd('"+h+"')\">"+a+"</button>")});_("#path",g);c()}else _("#path","");var n=function(a,b,N,p){var u=function(){var p=document.createElement("img");p.onload=function(){osd.inc();log.debug(a+" loaded");css(this.parentNode,
		"+loaded")};p.onclick=b;return p}(),c=document.createElement("li");css(c,"diapo "+N);c.appendChild(u);c.setAttribute("title",d(a));void 0!=p&&(u.id="diapo"+p,(F||w)&&append(c,'<input type="checkbox" id="chk'+p+'" n="'+p+'" onchange="walli.zwap('+p+')"/><label for="chk'+p+'"></label>'));(D[a]||[]).length&&append(c,'<span class="minicom">'+(999<D[a].length?Math.floor(D[a].length/1E3)+"K+":D[a].length)+"</span>");f.appendChild(c);u.src="?!=mini&file="+encodeURIComponent(a)};e=a.files;U=a.dirs;D=a.coms;
		N&&N();osd.start(e.length+U.length);a.dirs.forEach(function(a){n(a,function(){k(a)},"dir")});a.files.forEach(function(a,b){setTimeout(function(){n(a,function(){walli.show(b,0)},"",b)},b)});a.files.length&&F&&b("all");V();t();w&&_("#diag")&&walli.diag();E[r]&&y(0,0)})}function g(a,b){if(W[a]){var u=q.clientWidth,f=q.clientHeight,p=W[a],c=p.h,d=p.w;d>u&&(d=u,c=Math.floor(d*(p.h/p.w)));c>f&&(c=f,d=Math.floor(c*(p.w/p.h)));css(m[a],{width:d+"px",height:c+"px",left:Math.floor((u-d)/2+2*u*b)+"px",top:Math.floor((f-
		c)/2)+"px"})}}function K(){P&&(G&&clearTimeout(G),G=setTimeout(walli.next,1E3*H))}function B(a){H+=a;0>H&&(H=0);osd.loc("delay",{s:H})}function R(a){P!==a&&((P=a)?(K(),a=document.documentElement,a.requestFullscreen?a.requestFullscreen():a.mozRequestFullScreen?a.mozRequestFullScreen():a.webkitRequestFullScreen&&a.webkitRequestFullScreen(),css("#bplay","+active"),css("#view","+play"),osd.loc("play")):(G&&clearTimeout(G),G=!1,css("#bplay","-active"),css("#view","-play"),osd.loc("stop")))}function A(a){X!==
		a&&(X=a,log.debug("switch to "+a+" mode"),n=!0,"tof"==X?(C&&(clearInterval(C),C=!1),css(I,"+active"),css("#thumb","-active")):(n=!1,R(!1),osd.hide(),css(m[0],""),css(m[1],""),css(I,"-active"),css("#thumb","+active"),t()),V())}function $(a){var b=_("#diapo"+l).parentNode,c=_("#minicom"+l);c&&b.removeChild(c);0<a&&append(b,'<span id="minicom'+l+'" class="minicom">'+a+"</span>")}function Y(a,b){if(b||n&&Q!==a)Q=a,n&&(css(m[1-h],""),g(1-h,2)),a?(css("#bcom","+active"),css(I,"+com"+(b?"fix":"")),hash.set("com",
		1)):(css("#bcom","-active"),css(I,"-com"),css(I,"-comfix"),hash.del("com")),n&&setTimeout(function(){g(h,0)},550)}function Z(a){var b="",c=D[a];c&&c.length?(c.forEach(function(c,p){b+="<li><header>"+c.who+' <span title="'+c.when.replace("T"," ")+'">'+loc.reldate(c.when)+"</span></header><content>"+c.what.replace("\n","<br/>")+"</content>"+(c.own?'<button class="del" onclick="walli.rmcom(\''+a.replace("'","\\'")+"',"+c.id+')">'+loc.bdel+"</button>":"")+"</li>"}),_(L,b),L.scrollTop=L.scrollHeight,_("#comcount",
		999<c.length?Math.floor(c.length/1E3)+"K+":c.length)):(_(L,loc.nocom),_("#comcount","0"))}function ca(a,b){ajax({type:"POST",url:"?!=comment",data:{file:e[l],who:a,what:b},ok:function(a){D[a.file]=a.coms;Z(e[l]);$(a.coms.length);J.value=""},error:osd.error})}function da(a,b){ajax({type:"POST",url:"?!=uncomment",data:{file:a,id:b},ok:function(a){D[a.file]=a.coms;Z(e[l]);$(a.coms.length)},error:osd.error})}function v(a){a&&a.stopPropagation&&a.stopPropagation()}function V(){n?hash.set("f",e[l]):r?hash.set("f",
		r):hash.del("f")}function aa(){var a=hash.get("f"),b=hash.get("com"),c=/^(.+\/)([^\/]*)$/.test(a)?RegExp.$1:"/",f=RegExp.$2;b?Y(!0,!0):Q&&Y(!1,!0);return a.length?(b=function(){var b=e.indexOf(a),c=0;n&&b!=l&&(c=b<l?-1:1);-1!=b?walli.show(b,c):A("thumb")},c!=r?k(c,b):f?b():A("thumb"),!0):!1}function y(a,b,c){if(!n){var f=E[r]?E[r]:{x:0,y:0,n:0};c&&(f.x=a,f.y=b);f.o&&css(f.o,"-cursor");var p=_("#diapos li"),d=_("#diapos"),h=d.getBoundingClientRect(),g=[],k=!1,m=-1,e,l,q;if(p.length){for(;++m<p.length;)if(e=
		p[m].getBoundingClientRect(),e={t:e.top-h.top+d.scrollTop,w:e.width,h:e.height},e.w){l=Math.round(e.t+e.h/2);if(!1===k||l>k)k=l,g.push([]),q=g.length-1;g[q].push({t:e.t,b:e.t+e.h,n:m});c||f.n!=m||(f={x:a+g[q].length-1,y:b+q,n:m})}f.y>q?f.y=0:0>f.y&&(f.y=q);f.x>=g[f.y].length?f.x=0:0>f.x&&(f.x=g[f.y].length-1);l=g[f.y][f.x];f.o=p[l.n];f.n=l.n;e={t:d.scrollTop,b:d.scrollTop+h.height,h:h.height};q=e.t;l.b>e.b?q=l.b-e.h+30:l.t<e.t&&(q=l.t-30);d.scrollTop=q;css(f.o,"+cursor")}E[r]=f}}var H=5,G=!1,C=!1,
		T,r,e=[],U=[],x=[],l=!1,h=0,m=[],W=[],I,M,J,S=0,P=!1,n,q,z={},D=[],Q,X,L,ba,w=!1,F=!0,E={};return{setup:function(a){ba=a.comments;T=a.refresh;w=a.god;F=a.zip;I=_("#view");L=_("#coms");q=_("#slide");q.onmousewheel=function(a){n&&(0>(a.wheelDelta||a.detail/3)?walli.prev():walli.next(),a.preventDefault())};var b=function(){m[h].className="animated";m[h].style.left=z.l+"px";z={}};q.onmousedown=q.ontouchstart=function(a){a.preventDefault();a.touches&&(a=a.touches[0]);m[h].className="touch";z={d:!0,x:a.pageX,
		l:parseInt(m[h].style.left,10),h:setTimeout(b,1E3)}};q.onmousemove=q.ontouchmove=function(a){z.d&&(a.preventDefault(),a.touches&&(a=a.touches[0]),a=a.pageX-z.x,m[h].style.left=z.l+a+"px",80<Math.abs(a)&&(clearTimeout(z.h),z={},m[h].className="animated",80<a?walli.prev():walli.next()))};q.onmouseup=q.onmouseout=q.ontouchend=q.ontouchcancel=function(a){a.preventDefault();z.d&&(clearTimeout(z.h),b())};m=[_("#img0"),_("#img1")];q.ondragstart=m[0].ondragstart=m[1].ondragstart=function(a){a.preventDefault();
		return!1};window.onresize=function(){n&&(m[1-h].className="",g(1-h,1),g(h,0));c()};window.onorientationchange=function(){var a=_("#viewport"),b=window.orientation||0;a.setAttribute("content",90==b||-90==b||270==b?"height=device-width,width=device-height,initial-scale=1.0,maximum-scale=1.0":"height=device-height,width=device-width,initial-scale=1.0,maximum-scale=1.0");n&&g(h,0)};hotkeys.add("CTRL+D",function(){css("#log","*active")},!0).add(["SPACE","ENTER"],walli.toggleplay).add("C",walli.togglecom).add("HOME",
		walli.first).add("LEFT",walli.prev).add("RIGHT",walli.next).add("UP",walli.up).add("DOWN",walli.down).add("PAGEUP",walli.pgup).add("PAGEDOWN",walli.pgdown).add("END",walli.last).add(["ESC","BACKSPACE"],walli.back).add("DOWN",function(){!n&&e.length&&walli.show(0)}).add(["?","H"],function(){css("#help","*active")}).add("PLUS",function(){B(1)}).add("MINUS",function(){B(-1)});_("#bprev").onclick=walli.prev;_("#bnext").onclick=walli.next;_("#bplay").onclick=walli.toggleplay;_("#bthumb").onclick=walli.thumb;
		_("#bzip").onclick=walli.dlzip;F||(_("#bzip").className="hide");ba&&(M=_("#who"),J=_("#what"),J.onfocus=M.onfocus=function(){R(!1)},_("#comments").onclick=v,_("#bcom").onclick=walli.togglecom,_("#bsend").onclick=walli.sendcom);w?(_("#blogout").onclick=walli.logout,FormData?(_("#iupload").onchange=function(a){a=new FormData;for(var b=0,c=this.files,f,d=0;d<c.length;++d)f=c[d],b+=f.size,a.append("file"+d,f);if(confirm(loc.tpl("uploadfiles",{z:loc.size(b),nb:c.length}))){var e=new XMLHttpRequest;e.open("POST",
		"walli.php?!=img&&path="+r);e.onload=function(){if(200==e.status){var a=JSON.parse(e.responseText);osd.loc("uploaded",{nb:a.added});a.added&&k(r)}else osd.error("error "+e.status)};e.upload.onprogress=function(a){event.lengthComputable&&osd.set(a.loaded,a.total)};e.send(a)}},_("#bupload").onclick=function(){_("#iupload").click()}):css("#bupload",{diplay:"none"}),_("#bdel").onclick=walli.del,_("#bdiag").onclick=walli.switchdiag,_("#bflush").onclick=walli.flush,_("#bmkdir").onclick=walli.mkdir):a.admin&&
		(_("#blogin").onclick=walli.login);log.info("show on!");var d=_("#intro");if(d){var f=setTimeout(function(){css(d,"hide")},5E3);d.onclick=function(){clearTimeout(f);css(d,"hide")}}aa()||(A("thumb"),k("/"));hash.onchange(aa)},login:function(){document.location="?login"+document.location.hash},logout:function(){document.location="?logout"+document.location.hash},del:function(){if(w){var a=e.filter(function(a,b){return-1!=x.indexOf(b)});x.length?ajax({type:"POST",url:"?!=del",data:{files:a.join("*")},
		ok:function(a){osd.loc("deleted",{nb:a.deleted});a.deleted&&k(r)},error:osd.error}):osd.error(loc.noselection)}},switchdiag:function(){var a=_("#diag");a?document.body.removeChild(a):walli.diag()},diag:function(){w&&ajax({url:"?!=diag",data:{path:r},ok:function(a){var b=_("#diag"),c="<ul>",d;for(d in a.stats)c+='<li class="stat">'+("size"==d?loc.size(a.stats[d]):a.stats[d]+" "+d)+"</li>";for(d in a.checks)c+='<li class="'+(a.checks[d]?"ok":"bad")+'">'+d+(a.checks[d]?" enabled":" disabled")+"</li>";
		b||(b=document.createElement("div"),b.id="diag",b.onclick=function(){document.body.removeChild(b)},document.body.appendChild(b));b.innerHTML=c},error:osd.error})},flush:function(){w&&ajax({url:"?!=flush",ok:function(a){osd.loc("flushed",{nb:a.flushed})},error:osd.error})},mkdir:function(){var a;w&&(a=prompt(loc.mkdir))&&ajax({type:"POST",url:"?!=mkdir",data:{dir:a,path:r},ok:function(){k(r)},error:osd.error})},dlzip:function(){var a=x.length?e.filter(function(a,b){return-1!=x.indexOf(b)}):e;F&&a.length?
		(_("#bzip",loc.zip),ajax({type:"POST",url:"?!=zip",data:{files:a.join("*")},ok:function(a){document.location="?!=zip&zip="+a.zip;walli.zwap()},error:osd.error})):osd.loc("nozip")},zwap:function(a){void 0!=a&&(-1==x.indexOf(a)?x.push(a):x=x.filter(function(b){return b!=a}));x.length?(b("selected",loc.tpl("dlsel",{nb:x.length})),w&&(_("#bdel").className="")):(b("all",loc.dlall),w&&(_("#bdel").className="hide"))},thumb:function(){A("thumb")},show:function(a,b,c){e.length&&(l=0>a?e.length+a:a>=e.length?
		a%e.length:a,n||A("tof"),css("#mask","+active"),s(l,function(a,c){css("#mask","-active");n?h=1-h:b=0;W[h]={w:c.width,h:c.height};q.removeChild(m[h]);m[h].src=c.src;if(b)css(m[h],0>b?"left":"right"),g(h,b),q.appendChild(m[h]),g(h,0),css(m[h],"animated center"),css(m[1-h],"animated "+(0<b?"left":"right")),g(1-h,-b);else{css(m[h],"");var k=position("#diapo"+l);css(m[h],{width:k.width+"px",height:k.height+"px",left:k.left+"px",top:k.top+"px"});q.appendChild(m[h]);css(m[h],"animated");g(h,0)}K();V();osd.info(d(e[l])+
		" <sup>"+(l+1)+"/"+e.length+"</sup>");1<e.length&&s((l+1)%e.length,function(){})}),Z(e[l]))},next:function(a){v(a);n?walli.show(l+1,1):y(1,0)},prev:function(a){v(a);n?walli.show(l-1,-1):y(-1,0)},down:function(a){v(a);n?walli.show(l+1,1):y(0,1)},up:function(a){v(a);n?walli.show(l-1,-1):y(0,-1)},pgdown:function(a){v(a);n?walli.show(l+5,1):y(0,5)},pgup:function(a){v(a);n?walli.show(l-5,-1):y(0,-5)},first:function(a){v(a);n?walli.show(0,-1):y(0,0,!0)},last:function(a){v(a);n?walli.show(-1,1):y(-1,-1,
		!0)},play:function(a){v(a);e.length&&(n||walli.show(l,0),A("tof"))},stop:function(a){v(a);A("thumb")},toggleplay:function(a){v(a);n?R(!P):(E[r]||y(0,0,!0),E[r].o&&E[r].o.children[0].click(a))},togglecom:function(a){a&&a.stopPropagation();Y(!Q)},sendcom:function(){1>M.value.length?(osd.loc("emptywho"),M.focus()):1>J.value.length?(osd.loc("emptywhat"),J.focus()):ca(M.value,J.value)},rmcom:function(a,b){da(a,b)},back:function(){if(n)return A("thumb");var a=r.split("/");1<a.length&&k(a.slice(0,a.length-
		2).join("/"))},cd:function(a){A("thumb");k(a)}}}();
	</script>
	<script>
		ready(function(){
			walli.setup({
