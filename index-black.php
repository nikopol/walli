...
<?php } ?>
	...
	<script>
		...
		nozip:"rien \u00e0 t\u00e9l\u00e9charger",updir:"",uploadfiles:"poster %nb image%s (%z octets) ?",flushed:"%nb fichier%s supprim\u00e9%s",uploaded:"%nb image%s ajout\u00e9e%s",deleted:"%nb image%s effac\u00e9e%s",mkdir:"nom du dossier ?",delay:"le diaporama passe \u00e0 %ss",exifnotfound:"aucune information disponible"}},loc,setlocale=function(c){var b,v;loc=locales[c]?locales[c]:locales.en;loc.reldate=function(d){var b=(new Date).getTime();d=(new Date(d)).getTime();b=(b-d)/1E3;d=function(b,d){d=
		Math.round(d);return b.replace("%d",d).replace("%s",1<d?"s":"")};return 60>b?loc.date.now:3600>b?d(loc.date.min,b/60):86400>b?d(loc.date.hour,b/3600):172800>b?loc.date.yesterday:604800>b?d(loc.date.day,b/86400):2592E3>b?d(loc.date.week,b/604800):d(loc.date.month,b/2592E3)};loc.size=function(b){return 2048>b?b+"b":1E6>b?Math.round(b/1024)+"kb":1E9>b?Math.round(b/1E6)+"M":Math.round(b/1E9)+"G"};loc.tpl=function(b,c){var n=loc[b],h;for(h in c)n=n.replace(RegExp("%"+h,"g"),c[h]),"nb"==h&&(n=n.replace(RegExp("%s",
		"g"),1<c.nb?"s":""));return n};if(loc.title)for(b in loc.title)(v=_("#"+b))&&v.setAttribute("title",loc.title[b]);if(loc.holder)for(b in loc.holder)(v=_("#"+b))&&v.setAttribute("placeholder",loc.holder[b]);if(loc.text)for(b in loc.text)_("#"+b,loc.text[b]);c="<table><tr><th>"+loc.help.th.join("</th><th>")+"</th></tr>";for(b in loc.help.tr)c+="<tr><td>"+loc.help.tr[b].join("</td><td>")+"</td></tr>";_("#help",c)};ready(function(){setlocale(hash.get("lang")||navigator.language)});
		var log=function(){var c,b,v=(new Date).getTime(),d={debug:1,info:2,warn:3,error:4},w=hash.get("log"),n=w&&d[w]?d[w]:0;if(!n)return{debug:function(){},info:function(){},warn:function(){},error:function(){}};console.log?c=function(b,u){if(d[b]>=n){var c,v=("     "+b).substr(-5)+"|";for(c in u)console.log(v+u[c])}}:(ready(function(){append(document.body,'<div id="log"></div>');b=_("#log")}),c=function(c,u){if(b&&d[c]>=n){var M,w=("000000"+((new Date).getTime()-v)).substr(-6);for(M in u)append(b,'<div class="'+
		c+'"><span class="timer">'+w+"</span>"+u[M].replace(" ","&nbsp;")+"</div>");b.scrollTop=b.scrollHeight}});return{debug:function(){c("debug",arguments)},info:function(){c("info",arguments)},warn:function(){c("warn",arguments)},error:function(){c("error",arguments)}}}(),osd;
		osd=function(){var c,b,v,d,w,n,h=!1;ready(function(){c=_("#osd");b=_("#progress")});return{hide:function(){h&&(h=clearTimeout(h));css(c,"-active")},show:function(){css(c,"+active")},error:function(b){log.error(b);osd.info(b,"error",5E3)},info:function(b,d,n){_(c,b).className=d||"";h&&clearTimeout(h);osd.show();h=setTimeout(osd.hide,n||3E3)},loc:function(b,d){osd.info(loc.tpl(b,d))},start:function(c,h,w){v=w||"%v/%m";d=c;n=h;d&&(css(b,"+active"),osd.set(0))},set:function(c,h){h&&h!=d&&(d=h,css(b,"+active"));
		w=c;_("#progresstext",v.replace(/%v/,c).replace(/%m/,d));c>=d&&(css(b,"-active"),n&&n());_("#progressbar").style.width=d?Math.floor(position("#progress").width*c/d)+"px":0},inc:function(){osd.set(++w)}}}();var walli;
		walli=function(){function c(){var a=position("#thumbbar");_("#diapos").style.top=a.top+a.height+"px"}function b(a,q){G&&(_("#bzip",q).className=a,c())}function v(a,q){var E=l[a],b,s=Date.now();b=new Image;S++;b.onload=function(b){b||(b=window.event);log.info("image #"+a+" "+E+" loaded in "+(Date.now()-s)/1E3+"s");q(a,b.target||b.srcElement);S--};b.onerror=function(){log.error("error loading "+("image #"+a+" "+E));q(a,null);S--};b.src=encodeURIComponent(E)}function d(a){a=/([^\/]+)\/$/.test(a)?RegExp.$1:
		a.replace(/^.*\//g,"");return a.replace(/\.[^\.]*$/,"").replace(/[\._\|]/g," ")}function w(a){return 999<a?Math.floor(a/1E3)+"K+":a}function n(){T&&!D&&(D=setInterval(function(){log.debug("refresh required");ajax("?!=count&path="+r,function(a){U.length==a.dirs&&l.length==a.files||h(r)})},1E3*T))}function h(a,q){D&&(clearInterval(D),D=!1);log.debug("loading path "+(a||"/"));b("hide",loc.dlall);y&&(_("#bdel").className="hide");z=[];osd.hide();ajax("?!=ls&path="+a,function(a){var e=_("#diapos","");r=
		a.path;log.info((r||"/")+"loaded with "+a.dirs.length+" subdirs and "+a.files.length+" files found");if(r.length){var s=r.replace(/[^\/]+\/$/,"/"),P=document.createElement("li");css(P,"diapo up loaded");P.setAttribute("title",loc.updir);P.onclick=function(){h(s)};e.appendChild(P);var g="",m="";r.split("/").forEach(function(a){a&&(g+=a+"/",m+="<button onclick=\"walli.cd('"+g+"')\">"+htmlencode(a)+"</button>")});_("#path",m);c()}else _("#path","");var k=function(a,b,q,s){var E=function(){var q=document.createElement("img");
		q.onload=function(){osd.inc();log.debug(a+" loaded");css(this.parentNode,"+loaded")};q.onclick=b;return q}(),c=document.createElement("li");css(c,"diapo "+q);c.appendChild(E);c.setAttribute("title",d(a));void 0!=s&&(E.id="diapo"+s,(G||y)&&append(c,'<input type="checkbox" id="chk'+s+'" n="'+s+'" onchange="walli.zwap('+s+')"/><label for="chk'+s+'"></label>'));(H[a]||[]).length&&append(c,'<span class="minicom">'+w(H[a].length)+"</span>");e.appendChild(c);E.src="?!=mini&file="+encodeURIComponent(a)};
		l=a.files;U=a.dirs;H=a.coms;q&&q();osd.start(l.length+U.length);a.dirs.forEach(function(a){k(a,function(){h(a)},"dir")});a.files.forEach(function(a,q){setTimeout(function(){k(a,function(){walli.show(q,0)},"",q)},q)});a.files.length&&G&&b("all");V();n();y&&_("#diag")&&walli.diag();F[r]&&A(0,0)})}function u(a,q){if(W[a]){var b=p.clientWidth,e=p.clientHeight,s=W[a],c=s.h,d=s.w;d>b&&(d=b,c=Math.floor(s.h/s.w*d));c>e&&(c=e,d=Math.floor(s.w/s.h*c));css(k[a],{width:d+"px",height:c+"px",left:Math.floor((b-
		d)/2+b*q*2)+"px",top:Math.floor((e-c)/2)+"px"})}}function M(){Q&&(I&&clearTimeout(I),I=setTimeout(walli.next,1E3*J))}function aa(a){J+=a;0>J&&(J=0);osd.loc("delay",{s:J})}function ba(){css(document.body,"-sleep");document.body.onmousemove=document.body.ontouchstart=function(a){if(a.x!=ca||a.y!=da)ca=a.x,da=a.y,ba()};B&&clearTimeout(B);B=setTimeout(ia,1E3*ja)}function ia(){B&&(B=clearTimeout(B));css(document.body,"+sleep")}function X(a){Q!==a&&((Q=a)?(M(),a=document.documentElement,a.requestFullscreen?
		a.requestFullscreen():a.mozRequestFullScreen?a.mozRequestFullScreen():a.webkitRequestFullScreen&&a.webkitRequestFullScreen(),css("#bplay","+active"),css("#view","+play"),osd.loc("play"),ba()):(I&&(I=clearTimeout(I)),B&&(B=clearTimeout(B)),document.body.onmousemove=document.body.ontouchstart=!1,css(document.body,"-sleep"),css("#bplay","-active"),css("#view","-play"),osd.loc("stop")))}function C(a){Y!==a&&(Y=a,log.debug("switch to "+a+" mode"),m=!0,"tof"==Y?(D&&(clearInterval(D),D=!1),css(K,"+active"),
		css("#thumb","-active")):(m=!1,X(!1),osd.hide(),css(k[0],""),css(k[1],""),css(K,"-active"),css("#exif","-active"),css("#thumb","+active"),n()),V())}function ea(a){var q=_("#diapo"+f).parentNode,b=_("#minicom"+f);b&&q.removeChild(b);0<a&&append(q,'<span id="minicom'+f+'" class="minicom">'+a+"</span>")}function Z(a,b){if(b||m&&R!==a)R=a,m&&(css(k[1-g],""),u(1-g,2)),a?(css("#bcom","+active"),css(K,"+com"+(b?"fix":"")),hash.set("com",1)):(css("#bcom","-active"),css(K,"-com"),css(K,"-comfix"),hash.del("com")),
		m&&setTimeout(function(){u(g,0)},550)}function $(a){var b="",c=H[a];c&&c.length?(c.forEach(function(c,s){b+="<li><header>"+c.who+' <span title="'+c.when.replace("T"," ")+'">'+loc.reldate(c.when)+"</span></header><content>"+c.what.replace("\n","<br/>")+"</content>"+(c.own?'<button class="del" onclick="walli.rmcom(\''+a.replace("'","\\'")+"',"+c.id+')">'+loc.bdel+"</button>":"")+"</li>"}),_(N,b),N.scrollTop=N.scrollHeight,_("#comcount",w(c.length))):(_(N,loc.nocom),_("#comcount","0"))}function ka(a,
		b){ajax({type:"POST",url:"?!=comment",data:{file:l[f],who:a,what:b},ok:function(a){H[a.file]=a.coms;$(l[f]);ea(a.coms.length);L.value=""},error:osd.error})}function la(a,b){ajax({type:"POST",url:"?!=uncomment",data:{file:a,id:b},ok:function(a){H[a.file]=a.coms;$(l[f]);ea(a.coms.length)},error:osd.error})}function x(a){a&&a.stopPropagation&&a.stopPropagation()}function V(){m?hash.set("f",l[f]):r?hash.set("f",r):hash.del("f")}function fa(){var a=hash.get("f"),b=hash.get("com"),c=/^(.+\/)([^\/]*)$/.test(a)?
		RegExp.$1:"/",e=RegExp.$2;b?Z(!0,!0):R&&Z(!1,!0);return a.length?(b=function(){var b=l.indexOf(a),c=0;m&&b!=f&&(c=b<f?-1:1);-1!=b?walli.show(b,c):C("thumb")},c!=r?h(c,b):e?b():C("thumb"),!0):!1}function A(a,b,c){if(!m){var e=F[r]?F[r]:{x:0,y:0,n:0};c&&(e.x=a,e.y=b);e.o&&css(e.o,"-cursor");var s=_("#diapos li"),d=_("#diapos"),g=d.getBoundingClientRect(),h=[],k=!1,l=-1,f,n,p;if(s.length){for(;++l<s.length;)if(f=s[l].getBoundingClientRect(),f={t:f.top-g.top+d.scrollTop,w:f.width,h:f.height},f.w){n=Math.round(f.t+
		f.h/2);if(!1===k||n>k)k=n,h.push([]),p=h.length-1;h[p].push({t:f.t,b:f.t+f.h,n:l});c||e.n!=l||(e={x:a+h[p].length-1,y:b+p,n:l})}e.y>p?e.y=0:0>e.y&&(e.y=p);e.x>=h[e.y].length?e.x=0:0>e.x&&(e.x=h[e.y].length-1);n=h[e.y][e.x];e.o=s[n.n];e.n=n.n;f={t:d.scrollTop,b:d.scrollTop+g.height,h:g.height};p=f.t;n.b>f.b?p=n.b-f.h+30:n.t<f.t&&(p=n.t-30);d.scrollTop=p;css(e.o,"+cursor")}F[r]=e}}function ga(){m&&osd.info(d(l[f])+" <sup>"+(f+1)+"/"+l.length+"</sup>")}var J=5,ja=3,I=!1,D=!1,B=!1,T,r,l=[],U=[],z=[],
		f=!1,g=0,k=[],W=[],K,O,L,S=0,Q=!1,m,p,t={},H=[],R,Y,N,ha,y=!1,G=!0,ca=0,da=0,F={};return{setup:function(a){ha=a.comments;T=a.refresh;y=a.god;G=a.zip;K=_("#view");N=_("#coms");p=_("#slide");p.onmousewheel=function(a){m&&(0>(a.wheelDelta||a.detail/3)?walli.prev():walli.next(),a.preventDefault())};var b=function(){k[g].className="animated";k[g].style.left=t.l+"px";t={}};p.onmousedown=p.ontouchstart=function(a){a.preventDefault();a.touches&&(a=a.touches[0]);k[g].className="touch";t={c:t.c||0,d:!0,m:!1,
		x:a.pageX,l:parseInt(k[g].style.left,10),h:setTimeout(b,1E3)}};p.onmousemove=p.ontouchmove=function(a){t.d&&(t.m=!0,a.preventDefault(),a.touches&&(a=a.touches[0]),a=a.pageX-t.x,k[g].style.left=t.l+a+"px",80<Math.abs(a)&&(clearTimeout(t.h),t={},k[g].className="animated",80<a?walli.prev():walli.next()))};p.onmouseup=p.onmouseout=p.ontouchend=p.ontouchcancel=function(a){a.preventDefault();t.d&&(clearTimeout(t.h),t.c?(clearTimeout(t.c),b(),walli.toggleplay()):t.m?b():t.c=setTimeout(function(){b();walli.next()},
		250))};k=[_("#img0"),_("#img1")];p.ondragstart=k[0].ondragstart=k[1].ondragstart=function(a){a.preventDefault();return!1};window.onresize=function(){m&&(k[1-g].className="",u(1-g,1),u(g,0));c()};window.onorientationchange=function(){var a=_("#viewport"),b=window.orientation||0;a.setAttribute("content",90==b||-90==b||270==b?"height=device-width,width=device-height,initial-scale=1.0,maximum-scale=1.0":"height=device-height,width=device-width,initial-scale=1.0,maximum-scale=1.0");m&&u(g,0)};hotkeys.add("CTRL+D",
		function(){css("#log","*active")},!0).add(["SPACE","ENTER"],walli.toggleplay).add("C",walli.togglecom).add("HOME",walli.first).add("LEFT",walli.prev).add("RIGHT",walli.next).add("UP",walli.up).add("DOWN",walli.down).add("PAGEUP",walli.pgup).add("PAGEDOWN",walli.pgdown).add("END",walli.last).add(["ESC","BACKSPACE"],walli.back).add("DOWN",function(){!m&&l.length&&walli.show(0)}).add(["?","H"],function(){css("#help","*active")}).add(["T"],ga).add(["I"],walli.togglexif).add(["+"],function(){aa(1)}).add(["-"],
		function(){aa(-1)});_("#bprev").onclick=walli.prev;_("#bnext").onclick=walli.next;_("#bplay").onclick=walli.toggleplay;_("#bthumb").onclick=walli.thumb;_("#bzip").onclick=walli.dlzip;G||(_("#bzip").className="hide");ha&&(O=_("#who"),L=_("#what"),L.onfocus=O.onfocus=function(){X(!1)},_("#comments").onclick=x,_("#bcom").onclick=walli.togglecom,_("#bsend").onclick=walli.sendcom);y?(_("#blogout").onclick=walli.logout,FormData?(_("#iupload").onchange=function(a){a=new FormData;for(var b=0,c=this.files,
		q,d=0;d<c.length;++d)q=c[d],b+=q.size,a.append("file"+d,q);if(confirm(loc.tpl("uploadfiles",{z:loc.size(b),nb:c.length}))){var e=new XMLHttpRequest;e.open("POST","?!=img&path="+r);e.onload=function(){if(200==e.status){var a=JSON.parse(e.responseText);a.added&&h(r);osd.loc("uploaded",{nb:a.added})}else osd.error("error "+e.status)};e.upload.onprogress=function(a){event.lengthComputable&&osd.set(a.loaded,a.total)};e.send(a)}},_("#bupload").onclick=function(){_("#iupload").click()}):css("#bupload",{diplay:"none"}),
		_("#bdel").onclick=walli.del,_("#bdiag").onclick=walli.togglediag,_("#bflush").onclick=walli.flush,_("#bmkdir").onclick=walli.mkdir):a.admin&&(_("#blogin").onclick=walli.login);log.info("show on!");var d=_("#intro");if(d){var e=setTimeout(function(){css(d,"hide")},5E3);d.onclick=function(){clearTimeout(e);css(d,"hide")}}fa()||(C("thumb"),h("/"));hash.onchange(fa)},login:function(){document.location="?login"+document.location.hash},logout:function(){document.location="?logout"+document.location.hash},
		del:function(){if(y){var a=l.filter(function(a,b){return-1!=z.indexOf(b)});z.length?ajax({type:"POST",url:"?!=del",data:{files:a.join("*")},ok:function(a){a.deleted&&h(r);osd.loc("deleted",{nb:a.deleted})},error:osd.error}):osd.error(loc.noselection)}},togglexif:function(){css("#exif","*active");walli.exif()},exif:function(){var a=_("#exif.active");a.length&&(_(a,loc.load),ajax({url:"?!=exif",data:{file:l[f]},ok:function(b){var c,e,d;if(b&&b.exif)for(e in c="",b.exif){c+="<div><h3>"+e+"</h3><table>";
		for(d in b.exif[e])null!=b.exif[e][d]&&""!=b.exif[e][d]&&(c+="<tr><th>"+d+"</th><td>"+b.exif[e][d]+"</td></tr>");c+="</table></div>"}else c=loc.exifnotfound;_(a,c)},error:function(b){_(a,b)}}))},togglediag:function(){var a=_("#diag");a?document.body.removeChild(a):walli.diag()},diag:function(){y&&ajax({url:"?!=diag",data:{path:r},ok:function(a){var b=_("#diag"),c="<ul>",d;for(d in a.stats)c+='<li class="stat">'+("size"==d?loc.size(a.stats[d]):a.stats[d]+" "+d)+"</li>";for(d in a.checks)c+='<li class="'+
		(a.checks[d]?"ok":"bad")+'">'+d+(a.checks[d]?" enabled":" disabled")+"</li>";b||(b=document.createElement("div"),b.id="diag",b.onclick=function(){document.body.removeChild(b)},document.body.appendChild(b));b.innerHTML=c},error:osd.error})},flush:function(){y&&ajax({url:"?!=flush",ok:function(a){osd.loc("flushed",{nb:a.flushed})},error:osd.error})},mkdir:function(){var a;y&&(a=prompt(loc.mkdir))&&ajax({type:"POST",url:"?!=mkdir",data:{dir:a,path:r},ok:function(){h(r)},error:osd.error})},dlzip:function(){var a=
		z.length?l.filter(function(a,b){return-1!=z.indexOf(b)}):l;G&&a.length?(_("#bzip",loc.zip),ajax({type:"POST",url:"?!=zip",data:{files:a.join("*")},ok:function(a){document.location="?!=zip&zip="+a.zip;walli.zwap()},error:osd.error})):osd.loc("nozip")},zwap:function(a){void 0!=a&&(-1==z.indexOf(a)?z.push(a):z=z.filter(function(b){return b!=a}));z.length?(b("selected",loc.tpl("dlsel",{nb:z.length})),y&&(_("#bdel").className="")):(b("all",loc.dlall),y&&(_("#bdel").className="hide"))},thumb:function(){C("thumb")},
		show:function(a,b,c){l.length&&(f=0>a?l.length+a:a>=l.length?a%l.length:a,m||C("tof"),css("#mask","+active"),v(f,function(a,c){css("#mask","-active");m?g=1-g:b=0;W[g]={w:c.width,h:c.height};p.removeChild(k[g]);k[g].src=c.src;if(b)css(k[g],0>b?"left":"right"),u(g,b),p.appendChild(k[g]),u(g,0),css(k[g],"animated center"),css(k[1-g],"animated "+(0<b?"left":"right")),u(1-g,-b);else{css(k[g],"");var d=position("#diapo"+f);css(k[g],{width:d.width+"px",height:d.height+"px",left:d.left+"px",top:d.top+"px"});
		p.appendChild(k[g]);css(k[g],"animated");u(g,0)}M();V();ga();walli.exif();1<l.length&&v((f+1)%l.length,function(){})}),$(l[f]))},next:function(a){x(a);m?walli.show(f+1,1):A(1,0)},prev:function(a){x(a);m?walli.show(f-1,-1):A(-1,0)},down:function(a){x(a);m?walli.show(f+1,1):A(0,1)},up:function(a){x(a);m?walli.show(f-1,-1):A(0,-1)},pgdown:function(a){x(a);m?walli.show(f+5,1):A(0,5)},pgup:function(a){x(a);m?walli.show(f-5,-1):A(0,-5)},first:function(a){x(a);m?walli.show(0,-1):A(0,0,!0)},last:function(a){x(a);
		m?walli.show(-1,1):A(-1,-1,!0)},play:function(a){x(a);l.length&&(m||walli.show(f,0),C("tof"))},stop:function(a){x(a);C("thumb")},toggleplay:function(a){x(a);m?X(!Q):(F[r]||A(0,0,!0),F[r].o&&F[r].o.children[0].click(a))},togglecom:function(a){a&&a.stopPropagation();Z(!R)},sendcom:function(){1>O.value.length?(osd.loc("emptywho"),O.focus()):1>L.value.length?(osd.loc("emptywhat"),L.focus()):ka(O.value,L.value)},rmcom:function(a,b){la(a,b)},back:function(){if(m)return C("thumb");var a=r.split("/");1<a.length&&
		h(a.slice(0,a.length-2).join("/"))},cd:function(a){C("thumb");h(a)}}}();
	</script>
	<script>
		ready(function(){
			walli.setup({
